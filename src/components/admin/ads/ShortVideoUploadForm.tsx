...
const ShortVideoUploadForm = () => {
  ...
  const createShortMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        ...
        description: "The short video has been uploaded successfully."
      });
    },
    onError: (error) => {
  ...
  const handleFileUpload = async (
    ...
    try {
      ...
      const fileName = `${Date.now()}-short-${fileType}.${fileExt}`;
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="space-y-2">
          <Label htmlFor="title" className="text-white">Short Video Title *</Label>
          <Input 
            id="title" 
            name="title" 
        ...
        <div className="space-y-2">
          ...
          <Input 
            ...
            onChange={handleInputChange}
            placeholder="Duration in seconds (max 60)"
            max="60"
            className="bg-input border-border"
          />
        </div>
      ...
      <div className="flex items-center space-x-2">
        <input 
          ...
          className="rounded"
        />
        <Label htmlFor="is_featured" className="text-white">Featured Short</Label>
      </div>
